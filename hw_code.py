...
fprintf("Problem 2 program output")
r = [2,0.5,1]
v = [0.5,0.5,-0.5]
% Calculate h
h = cross(r,v)
...
fprintf("Problem 3 program output")
clear
% Cannonical units
u = 1
a = 6.3920
...
p = a*(1-ecc^2)
r0 = p/(1+ecc*cosd(theta))
% Using verical vector notation
r_pqw = [(r0*cosd(theta)); (r0*sind(theta)); 0]
v_pqw = sqrt(u/p)*[-sind(theta); (ecc+cosd(theta)); 0]
R = [
    (cosd(RAAN)*cosd(omega) - sind(RAAN)*sind(omega)*cosd(i_el)), ...
    (-cosd(RAAN)*sind(omega) - sind(RAAN)*cosd(omega)*cosd(i_el)), ...
    sind(RAAN)*sind(i_el);
    
    (sind(RAAN)*cosd(omega) + cosd(RAAN)*sind(omega)*cosd(i_el)), ...
    (-sind(RAAN)*sind(omega) + cosd(RAAN)*cosd(omega)*cosd(i_el)), ...
    -cosd(RAAN)*sind(i_el);
    
    sind(omega)*sind(i_el), ...
    cosd(omega)*sind(i_el), ...
    cosd(i_el)
];
r_ijk = R*r_pqw
v_ijk = R*v_pqw